...
boolean pushInstSecond(char *line, int *lInd, int lineCnt){
    ...
    case relative:
        ...
        if (wasDefined(operand, lineCnt, FALSE, 2)){  /*******************************************changed !*/
            ...
            if(AM == relative)
                (*instImage)[blankIC].wrd = blankIC - symTabRow.address + OS_MEM - 1;
            else
...
static word *scanInstParams(char *line, int *lInd, int lineCnt)
{
    ...
    if (operands[opAmnt-1] == NULL || opAmnt-1 > 2)
...
void pushInstsToFile(FILE *fd){
    ...
    for (i = 0; i < IC - OS_MEM; i++)
        fprintf(fd, "%04d %03x %d %c\n", i + OS_MEM, ((*instImage)[i].wrd & 0xFFF), (*instImage)[i].wrd, (*instImage)[i].ARE);
    return;
}