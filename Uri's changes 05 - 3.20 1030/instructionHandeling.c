...
static inst instsList[INSTRUCTIONS] = {
    ...
    {"mov",0,0,{0,1,3},{1,3,-1},2}, {"cmp",1,0,{0,1,3},{0,1,3},2}, {"add",2,10,{0,1,3},{1,3,-1},2},
    {"sub",2,11,{0,1,3},{1,3,-1},2}, {"lea",4,0,{1,-1},{1,3,-1},2}, 
    /* the one operand instructions */
    {"clr",5,10,{-1},{1,3,-1},1}, {"not",5,11,{-1},{1,3,-1},1}, {"inc",5,12,{-1},{1,3,-1},1}, 
    {"dec",5,13,{-1},{1,3,-1},1}, {"jmp",9,10,{-1},{1,2,-1},1}, {"bne",9,11,{-1},{1,2,-1},1}, 
    {"jsr",9,12,{-1},{1,2,-1},1}, {"red",12,0,{-1},{1,3,-1},1}, {"prn",13,0,{-1},{0,1,3},1}, 
    /* the zero operand instructions */
    {"rts",14,0,{-1},{-1},0}, {"stop",15,0,{-1},{-1},0}
};

static enum addressing {immidiate, direct, relative, registerDirect};
...
static word *scanInstParams(char *line, int *lInd, int lineCnt){
    ...
    if(!isAlloc(operands) || !isAlloc(memory) || (operands[0] = readWord(line, lInd)) == NULL){ /* reading the operation name */
        if(operands != NULL)
    ...
    while((operands[opAmnt] = readWord(line, lInd)) != NULL && !strcmp(operand[opAmnt], "\0")){ /* reading the operands of the command to an array */
    ...
    if(operands[opAmnt] == NULL || opAmnt-1 > 2){ /* could not read the last operand or there are more than 2 operands */
        freeArr(operands, opAmnt)
        free(memory);
        if(opAmnt-1 > 2){
    ...
    if(!isValidOpsAmount(operationInd, opAmnt-1, lineCnt)){ /* the operands amount does not correspond to the needed amount of operands */
        freeArr(operands, opAmnt)
        free(memory);
        return NULL;
    }
